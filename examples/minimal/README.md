...
The smallest valid call to this module: grant the running principal Reader on a resource group, using
required inputs only (`scope`, `principal_ids`, `role_names`). The environment comes from the Terraform
...
| Name | Description |
|------|-------------|
| <a name="output_role_assignment_ids"></a> [role\_assignment\_ids](#output\_role\_assignment\_ids) | Map of assignment key to role assignment id. |
